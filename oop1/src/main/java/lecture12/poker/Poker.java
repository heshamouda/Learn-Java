...
public class Poker {
	...
	public static void sortCards(Card[] cards) {
		...
		for (int i = 0; i < 5; i++) {
			position = i;
			maxValue = cards[position].getValue().getValue(); // First .getValue() returns the CardValue --> Second .getValue() returns the int value of a CardValue
			for (int j = i + 1; j < 5; j++) {
				if (maxValue < cards[j].getValue().getValue()) {
					position = j;
					maxValue = cards[j].getValue().getValue();
				}
			}
			Card temp = cards[i];
	...
	public static String testCards(Card[] cards) {
		...
		if (sameColor(cards)) {
			// highest card ace
			if (isStreet(cards)) {
				if (cards[0].getValue() == CardValue.ACE) {
	...
	protected static boolean isStreet(Card[] cards) {
		return (cards[0].getValue().getValue() - 1 == cards[1].getValue().getValue())
				&& (cards[1].getValue().getValue() - 1 == cards[2].getValue().getValue())
				&& (cards[2].getValue().getValue() - 1 == cards[3].getValue().getValue())
				&& (cards[3].getValue().getValue() - 1 == cards[4].getValue().getValue());
	}
